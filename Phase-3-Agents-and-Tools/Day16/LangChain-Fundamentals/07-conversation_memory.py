...
prompt = ChatPromptTemplate.from_messages([
    ("system", "You are a friendly assistant. Keep answer to one sentence."),
...
def run_conversation(model, label):
    ...
    print(f"---{label}---")
    for user_text in ["Hi! My name is Tushar.", "What is my name?"]:
...
if not os.getenv("GROQ_API_KEY"):
    ...
    print("With a key, the GROQ model answers turn 2 with 'Your name is Tushar.'")
else:
